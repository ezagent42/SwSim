...
本规范定义三个阶段的文件格式：**模板**（`.socialware.md`）、**已开发 App**（`app-store/{app-id}.app.md`）、**已安装实例**（`rooms/{room}/contracts/{app-id}.app.md`）。

## 文件命名

- 模板: `{descriptive-name}.socialware.md`（如 `two-role-submit-approve.socialware.md`）
- 已开发 App: `{app-id}.app.md`（如 `doc-review-workflow.app.md`），app-id 为描述性名称
- 已安装实例: 与 app-store 中同名，复制到 Room 的 `contracts/` 目录
- 名称解耦: 模板名 ≠ app-id ≠ namespace，由用户在各阶段独立选择

## 身份格式

- 格式: `{username}:{nickname}@{namespace}`（如 `alice:Alice@local`）
- namespace 默认为 `local`（模拟环境）
- 无 `@` 前缀——`@` 仅作 username:nickname 和 namespace 的分隔符
- 文件名: `identities/{username}@{namespace}.json`（如 `identities/alice@local.json`）——文件名省略 nickname（避免 `:` 在 Windows/WSL 文件系统问题），但 JSON 内 `entity_id` 使用完整格式
