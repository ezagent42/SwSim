...
过程:
- 选择一份模板（`simulation/socialware/*.socialware.md`）
- 为 App 选择一个短名称（AppName），与开发者身份和模板名组合生成 App-ID
  - App-ID 格式: `{AppName}.{DeveloperName}.{SocialwareName}`
  - 例: `doc-review.alice.two-role-submit-approve`
- 给每个动作绑定具体的工具
- 声明跨契约引用（依赖/委托/资源）
- 生成 `{app-id}.app.md` 保存到 app-store
- 运行注册脚本将 App 登记到 `simulation/app-store/registry.json`
...
输入（只读）                              输出（App 制品）
─────────                               ─────────────
simulation/socialware/                   simulation/app-store/
  ...
  ★ 不修改此文件                            namespace 未定（install 阶段选）
                                         ...
                                         simulation/app-store/
                                           registry.json  ← 注册 App 信息
```

- **模板不变**: `simulation/socialware/{name}.socialware.md` 是 Socialware 产品，只读不改
- **App 输出到 app-store**: `simulation/app-store/{app-id}.app.md`
- **App-ID 格式**: `{AppName}.{DeveloperName}.{SocialwareName}`（如 `doc-review.alice.two-role-submit-approve`）
- **命名解耦**: 模板名、AppName、namespace 三者由用户在各阶段独立选择

## 流程

### Phase 1: 选择模板和命名

1. 列出可用模板（`simulation/socialware/*.socialware.md`），让用户选择
2. 让用户选择 AppName（短名称，如 `doc-review`）
3. 根据当前身份（DeveloperName = username 部分，即 `:` 前的部分）和模板名（SocialwareName = 模板文件名去掉 `.socialware.md`）自动生成 App-ID: `{AppName}.{DeveloperName}.{SocialwareName}`
4. 读取模板，展示概要（Role, Flow, Commitment, Arena）

### Phase 2: 填充 Bindings（§5）

5. 逐个动作填充，每次一个，**严格使用列表格式**（与 contract-spec.md 和模板格式一致）:
...
6. 如果有跨契约依赖，声明引用关系（依赖/委托/资源）
7. 具体的 mock 数据在 install 阶段处理

### Phase 4: 写入

8. 创建 `simulation/app-store/` 目录（如不存在）
9. 写入 `simulation/app-store/{app-id}.app.md`:
   - 头部: 状态 → `已开发`，App-ID，基于模板，`开发者: {当前身份}`
   - §1: 持有者保持 `_待绑定_`（不在此阶段填）
   - §5: 填入工具绑定
   - 不写 §6（安装阶段才写）
   - 不更新任何 Room 的 config.json 或 state.json
10. 运行注册脚本，将 App 登记到 `simulation/app-store/registry.json`:
   ```bash
   python3 .claude/skills/socialware-app-dev/scripts/register-app.py \
     --app-name "{AppName}" \
     --developer "{identity}" \
     --socialware "{SocialwareName}" \
...
- **一次一个 binding**
- **模板只读**: 永远不修改 `simulation/socialware/` 中的文件
- **输出到 app-store**: `simulation/app-store/{app-id}.app.md`
