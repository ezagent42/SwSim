...
- **输出**：`simulation/workspace/rooms/{name}/`（空 Room，含 config.json）
  - config.json 初始结构：`{ membership: {...}, socialware-app: { installed: [], roles: {} } }`
...
- **输出**：
  - `workspace/rooms/{room}/socialware-app/{AppName}.{DeveloperName}.{SocialwareName}.app.md`（状态：`已安装`，§1 已填持有者）
  - 更新 `config.json`（`socialware-app.installed` 追加条目）
...
| 同 Room 多 Socialware | 同一 Room 目录下多个 `socialware-app/{AppName}.{DeveloperName}.{SocialwareName}.app.md` |
