...
- **Template 名 ≠ App 名**：二者解耦。例如 `two-role-submit-approve.socialware.md` 安装后的 App 命名为 `da.app.md`。
...
- **输入**：组织需求描述
- **工具**：`/socialware-dev`
- **输出**：`simulation/contracts/{name}.socialware.md`
  - 状态：`模板`
  - §5（绑定信息）全部标记为 `_待实现_`
...
- **输出**：`workspace/app-store/{app-id}.app.md`（状态：`已开发`，§5 已填工具，§1 仍为 `_待绑定_`）
...
- **输出**：
  - `workspace/rooms/{room}/contracts/{app-id}.app.md`（状态：`已安装`，§1 已填持有者）
...
```json
{
  "socialware": {
    "installed": [
      {"app_id": "event-weaver", "namespace": "ew", "contract": "event-weaver.app.md", "template": "event-weaver.socialware.md"},
      {"app_id": "task-arena", "namespace": "ta", "contract": "task-arena.app.md", "template": "two-role-submit-approve.socialware.md"},
      {"app_id": "res-pool", "namespace": "rp", "contract": "res-pool.app.md", "template": "resource-pool.socialware.md"}
    ]
  }
}
```
...
| Contract Template 分发 | `simulation/contracts/{name}.socialware.md` |
| Room | `simulation/workspace/rooms/{name}/` 目录 |
| 同 Room 多 Socialware | 同一 Room 目录下多个 `contracts/{app-id}.app.md` |
