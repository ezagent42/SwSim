...
```
Socialware Dev          App Dev              Room              Users
     │                    │                   │                  │
     │  ① /socialware-dev │                   │                  │
     │──────────────────▶ │                   │                  │
     │  输出: Template     │                   │                  │
...
- **输出**：`simulation/workspace/rooms/{name}/`（空 Room，含 config.json）
  - config.json 初始结构：`{ members: [...], socialware: { installed: [] } }`

### 4.3 Socialware App Dev → Installed App

- **输入**：Template + Room + 角色/工具绑定决策
- **工具**：`/socialware-app-dev`
- **输出**：
  - `workspace/rooms/{room}/contracts/{ns}.app.md`（状态：`已绑定`）
  - 更新 `config.json`（`socialware.installed` 追加 namespace）

### 4.4 Socialware App Runtime → 用户交互
...
```
/switch @alice  →  以 alice 身份操作
/switch @bob    →  以 bob 身份操作
```
...
| 同 Room 多 Socialware | 同一 Room 目录下多个 `contracts/{ns}.app.md` |
