...
> **测试目标**：验证 `/socialware-app-dev`（工具绑定 → app-store + registry 注册）+ `/socialware-app-install`（从 registry 查询 → 角色绑定 → Room）完整流程
...
- **前置依赖**：`simulation/socialware/two-role-submit-approve.socialware.md` 存在
- **验证**：Skill 列出可用模板
- **验收标准**：
  - 模板列表包含 `two-role-submit-approve.socialware.md`（来自 `simulation/socialware/`）

### Step 2: 选择模板和 App-ID

- **操作**：选择模板=two-role-submit-approve，App-ID=`doc-audit.alice.two-role-submit-approve`（开发者 alice 选择 AppName=`doc-audit`）
- **前置依赖**：Step 1
- **验证**：Skill 展示模板概要（Role, Flow, Commitment, Arena）
- **验收标准**：
  - App-ID 格式为 `{AppName}.{DeveloperName}.{SocialwareName}`（此处为 `doc-audit.alice.two-role-submit-approve`）
  - AppName = 开发者自选名称（`doc-audit`）
  - DeveloperName = 开发者 Identity 的 username（`alice`，来自 `alice:Alice@local`）
  - SocialwareName = 模板名去掉 `.socialware.md` 扩展名（`two-role-submit-approve`）
  - 展示的概要与模板文件内容一致
...
- **验收标准**：
  ...
  - `simulation/app-store/registry.json` 中已注册该 App（包含 app-id、模板来源、开发者等元数据）
...
- **前置依赖**：Step 4（app-store 中有 `doc-audit.alice.two-role-submit-approve.app.md`），`rooms/doc-review/` 存在且有 alice 和 bob
- **验证**：Skill 列出可用 Room，并从 `simulation/app-store/registry.json` 查询可用 App
- **验收标准**：
  - Room 列表包含 `doc-review`
  - App 列表包含 `doc-audit.alice.two-role-submit-approve`（从 registry.json 查询得到）

### Step 7: 选择 Room、Namespace 并绑定角色

- **操作**：选择 Room=doc-review，App=`doc-audit.alice.two-role-submit-approve`，Namespace=da，R1(提交者) → alice:Alice@local，R2(审批者) → bob:Bob@local
- **前置依赖**：Step 6
...
- **操作**：确认模板名、App-ID 和 namespace 完全独立
- **前置依赖**：Step 8
- **验证**：
  - 模板文件名：`two-role-submit-approve.socialware.md`（位于 `simulation/socialware/`）
  - App-ID：`doc-audit.alice.two-role-submit-approve`（格式为 `{AppName}.{DeveloperName}.{SocialwareName}`）
  - App 文件名：`doc-audit.alice.two-role-submit-approve.app.md`（以 app-id 命名，格式为 `{AppName}.{DeveloperName}.{SocialwareName}.app.md`）
  - Namespace：`da`（在 install 阶段选择，可与 app-id 不同）
  - 三者（模板名、app-id、namespace）互相独立
- **验收标准**：同一模板可以用不同 AppName 多次开发（生成不同 app-id），也可以在不同 Room 以不同 namespace 安装
