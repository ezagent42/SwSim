...
> **前置依赖**：无（自包含）
> **测试 Skill**：`/socialware-dev` + `/room` + `/socialware-app-dev` + `/socialware-app-install` + `/socialware-app`
> **覆盖 Spec**：001-007
...
- **验收标准**：
  ...
  - §3 有 C1（审批者在规定时间内回复）
  - §4 有准入策略
  - §5 有每个 action 的 binding 骨架，Tool 全为 `_待实现_`
  - 扩展名 `.socialware.md`

### Step 2: 创建 Room

- **操作**：执行 `/room create project-e2e`，创建者 alice:Alice@local
- **前置依赖**：无
- **验证**：Room 目录结构完整
- **验收标准**：
...
- **操作**：执行 `/room join project-e2e bob:Bob@local`
- **前置依赖**：Step 2
- **验证**：bob 成功加入
- **验收标准**：
  - `config.json` 的 `membership.members` 包含 `"bob:Bob@local": "member"`
  - Identity 文件 `bob:Bob@local.json` 存在
  - `state.json` 的 `peer_cursors` 有 bob:Bob@local 条目
...
- **操作**：执行 `/socialware-app`，Room=project-e2e，Identity=alice:Alice@local
- **前置依赖**：Step 4
- **验证**：启动面板正确
- **验收标准**：
  - 显示角色 da:R1
...
- **验收标准**：
  ...
  - `flow_states["msg-001"]` = `{flow: "da:task_lifecycle", state: "submitted", subject_author: "alice:Alice@local"}`

### Step 7: Bob 查看收件箱

- **操作**：切换到 bob（`/switch bob:Bob@local`）
- **前置依赖**：Step 6
- **验证**：收件箱显示 Alice 的提交
- **验收标准**：
  - 收件箱：`[clock:1] alice:Alice@local 提交了任务: E2E 测试任务`
...
- **操作**：切换回 alice（`/switch alice:Alice@local`）
- **前置依赖**：Step 8
- **验证**：Alice 看到审批结果
- **验收标准**：
  - 收件箱：`[clock:2] bob:Bob@local 审批通过: 方案可行`
...
- **验收标准**：
  ...
  - `last_clock` = 2
  - `peer_cursors` 中 alice:Alice@local 和 bob:Bob@local 都有值
