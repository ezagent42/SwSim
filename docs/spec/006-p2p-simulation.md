...
| 身份验证 | 读取 `workspace/identities/{username}:{nickname}@local.json` |
...
8. 打印消息到终端                     9. watch-timeline 检测到变化
                                     10. 打印通知: "[clock:N] alice:Alice@local → ns:action"
                                     11. 用户执行 /inbox 查看详情
                                     12. 或直接执行后续动作
```
...
```bash
# 启动 2 个 peer（n 个 identity → 2n 个 pane）
.claude/skills/socialware-app/scripts/start-p2p.sh project-alpha alice:Alice@local bob:Bob@local

# 启动 3 个 peer
.claude/skills/socialware-app/scripts/start-p2p.sh project-alpha alice:Alice@local bob:Bob@local charlie:Charlie@local
...
每个 identity 占 2 个 pane（peer + watcher），n 个 identity 共 2n 个 pane：
```
┌─────────────────────┬─────────────────────┐
│ Peer: alice:Alice@local │ Peer: bob:Bob@local │   ← peer pane (Claude Code)
│ Room: project-alpha │ Room: project-alpha │
│ /socialware-app     │ /socialware-app     │
├─────────────────────┼─────────────────────┤
│ watcher (20%)       │ watcher (20%)       │   ← watcher pane (watch-timeline.sh)
│ 过滤自己的消息      │ 过滤自己的消息      │
└─────────────────────┴─────────────────────┘
```
...
- 再次运行（无 `--force`）会 attach 到已有 session
...
cd /path/to/SwSim
claude
# 然后输入: /socialware-app, 选择 room=project-alpha, identity=alice:Alice@local

# Terminal 2
cd /path/to/SwSim
claude
# 然后输入: /socialware-app, 选择 room=project-alpha, identity=bob:Bob@local
```
...
```
主 session:
  Agent A: /socialware-app room=project-alpha identity=alice:Alice@local
  Agent B: /socialware-app room=project-alpha identity=bob:Bob@local
...
14:23:05 [clock:3] alice:Alice@local → ew:branch.create
14:23:18 [clock:4] alice:Alice@local → ta:task.post
```
...
──────────────────────────────────────
  [clock:3] 🌿 alice:Alice@local 创建了分支: hotfix (ew:branch.create)
  [clock:4] 📋 alice:Alice@local 提交了任务: 实现认证 (ta:task.post)
```
...
══════════════════════════════════════════════════
  ...
  📬 3 条未读消息（自 clock=2 以来）:
    [clock:3] alice:Alice@local → ew:branch.create
    [clock:4] alice:Alice@local → ta:task.post
    [clock:5] alice:Alice@local → ta:task.submit
──────────────────────────────────────────────────
```
